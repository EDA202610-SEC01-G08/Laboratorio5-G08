...
def load_books(catalog):
    ...
    return catalog["books"], catalog["authors"]
...
def set_book_sublist(catalog, size):
    ...
    """
    books = catalog["books"]
    max_size = data_structure.size(catalog["books"])
...
def sort_books(catalog):
    ...
    if "book_sublist" not in catalog:
        max_size = data_structure.size(catalog["books"])
        catalog["book_sublist"] = data_structure.sub_list(catalog["books"], 0, min(10, max_size))

    sorted_books = catalog["book_sublist"]
    start_time = get_time()

    if sort_algorithm is None:
        raise ValueError("No sorting algorithm selected. Please select one before sorting.")
    ...
    if sort_algorithm == 1:
        ...
        raise ValueError("Invalid sort_algorithm value. Please select a valid sorting algorithm (1 to 5).")
